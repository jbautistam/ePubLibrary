...
namespace Bau.Libraries.LibEBook.Formats.ePub.Parser
{
	...
	public class ePubParserNCX
	{
		...
		/// </summary>
		internal static void Parse(OPFPackage package, string path)
		{
		...
		///		Interpreta la cabecera
		/// </summary>
		private static void ParseHeader(NCXFile ncx, MLNode mlNode)
		{
		...
		///		Interpreta el índice
		/// </summary>
		private static NavPointsCollection ParseIndex(MLNode mlNode)
		{
					...
					foreach (MLNode mlChild in mlNode.Nodes)
						if (mlChild.Name == NCXConstants.TagNavPoint)
						{
							NavPoint page = new NavPoint();
								...
								page.ID = mlChild.Attributes [NCXConstants.TagNavPointID].Value;
