...
namespace Bau.Controls.Tree
{
	...
	{ // Constantes
			private const string TextEmptyChild = "#¬282%¬¬¬€212#";
		// Delegados
		...
		// Variables privadas
			...
			private TreeNodeKeyCollection mlNodesKeys = new TreeNodeKeyCollection();
		...
		{ TreeNode trnNode;
				...
				// Si puede tener hijos le añade un nodo vacío
					if (blnWithChilds)
						trnNode.Nodes.Add(TextEmptyChild);
				// Devuelve el nodo
					return trnNode;
		}
		...
		///		Lanza un evento
		/// </summary>
		private void RaiseEvent(TreeEventType type, TreeNode trnNode)
		{ if (IsNodeWithKey(trnNode))
				switch (type)
		...
		/// </summary>
		private void CheckNodesRecursive(TreeNodeCollection mlNodes, int intIDType, int? intID, bool blnChecked)
		{ foreach (TreeNode trnNode in mlNodes)
		...
		/// </summary>
		private TreeNode SearchNodeRecursive(TreeNodeCollection mlNodes, int intIDType, int? intID)
		...
		{ TreeNodeKeyCollection mlNodesKeys = new TreeNodeKeyCollection();
		
				// Recorre el árbol buscando los nodos seleccionados
					foreach (TreeNode trnNode in Nodes)
						GetCheckedNodesRecursive(trnNode, mlNodesKeys);
		...
		///		Obtiene una colección con los nodos seleccionados
		/// </summary>
		private void GetCheckedNodesRecursive(TreeNode trnNode, TreeNodeKeyCollection mlNodesKeys)
		{ // Si el nodo está seleccionado, lo añade a la colección
				if (IsNodeWithKey(trnNode) && trnNode.Checked)
					mlNodesKeys.Add(trnNode.Tag as TreeNodeKey);
			// Recorre los nodos hijos
				foreach (TreeNode trnChild in trnNode.Nodes)
					GetCheckedNodesRecursive(trnChild, mlNodesKeys);
		}
		...
		{	TreeNode trnNode;
				...
				foreach (TreeNodeKey objNodeKey in mlNodesKeys)
					if (objNodeKey != null)
						{	// Busca el nodo (puede que se haya eliminado el elemento que lo representa)
							...
							// Lanza el evento de apertura de nodos
								if (trnNode != null && trnNode.Nodes.Count == 1 && trnNode.Nodes[0].Text == TextEmptyChild)
		...
		{	// Si se debe llamar al evento de cargar nodos
				if (e.Node != null && e.Node.Nodes != null && e.Node.Nodes.Count == 1 && 
						e.Node.Nodes[0].Text == TextEmptyChild)
