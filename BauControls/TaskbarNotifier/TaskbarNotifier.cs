...
namespace Bau.Controls.TaskbarNotifier
{
	...
	public class TaskbarNotifier : System.Windows.Forms.Form
	{
		...
		/// </summary>
		/// <param name="title">The string which will be shown as the title of the popup</param>
		...
		public void Show(string title, string strContent, int nTimeToShow, int nTimeToStay, int nTimeToHide)
		...
		{	SetBackgroundBitmap(new Bitmap(fileName), transparencyColor);
		}
		...
		/// </summary>
		/// <param name="fileName">Path of the 3-state Close button Bitmap on the disk (width must a multiple of 3)</param>
		/// <param name="transparencyColor">Color of the Bitmap which won't be visible</param>
		/// <param name="position">Location of the close button on the popup</param>
		/// <returns>Nothing</returns>
		public void SetCloseBitmap(string fileName, Color transparencyColor, Point position)
		{	SetCloseBitmap(new Bitmap(fileName), transparencyColor, position);
		}
