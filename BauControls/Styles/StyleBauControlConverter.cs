...
namespace Bau.Controls.Styles
{
	...
	public class StyleBauControlConverter : ExpandableObjectConverter
	{
		...
		{	string [] arrvalues = ((string) objValue).Split(',');
			ColorConverter objConverter = new ColorConverter();
			StyleBauControl objStyle = new StyleBauControl();
		
				try
					{	// Obtiene la cadena formateada con los valores numéricos
							objStyle.RoundCorners = int.Parse(arrvalues[0]);
							objStyle.AngleCorners = int.Parse(arrvalues[1]);
							objStyle.ShadowThickness = int.Parse(arrvalues[2]);
							objStyle.BorderWidth = int.Parse(arrvalues[3]);
						// Obtiene la cadena formateada con los colores
							objStyle.BorderColor = (Color) objConverter.ConvertFromString(arrvalues[4]);
							objStyle.BackgroundColor = (Color) objConverter.ConvertFromString(arrvalues[5]);
							objStyle.BackgroundGradientColor = (Color) objConverter.ConvertFromString(arrvalues[6]);
							objStyle.ShadowColor = (Color) objConverter.ConvertFromString(arrvalues[7]);
							objStyle.ForeColor = (Color) objConverter.ConvertFromString(arrvalues[8]);
						// Obtiene la cadena formateada con el modo del gradiante
							objStyle.GradientMode = (StyleBauControl.GroupBoxGradientMode) int.Parse(arrvalues[9]);
						// Devuelve el estilo
							return objStyle;
					}
				catch
