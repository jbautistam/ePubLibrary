...
{ 
	...
	{	// Delegados públicos
		...
		{	Remove(objColControls[index].Control);
		}
		...
		{ // Busca entre los controles el que se encuentra en esta posición
				for (int index = 0; index < objColControls.Count; index++)
					if (objColControls[index].Top <= intTop && 
							objColControls[index].Top + objColControls[index].Control.Height >= intTop)
						return index;
